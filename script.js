...
modal?.addEventListener("click", (e) => {
  ...
  // Kuormanumero *ei* varata/nosteta selaimessa.
  // Apps Script asettaa numeron vasta onnistuneen rivitallennuksen yhteydessä
  // ja palauttaa sen vastauksessa (loadNo), jolloin se jää näkyviin raporttiin.
  ...
  function openModal(row) {
    if (!row) return;
    const d = row.data || {};
        const lnTxt = (d.loadNo != null && String(d.loadNo).trim() !== "") ? String(d.loadNo).trim() : "";
    if (loadNoLabel) loadNoLabel.textContent = lnTxt ? lnTxt : "—";
    if (loadNoBox) loadNoBox.value = lnTxt;
    const today = isoDate(new Date());
  ...
  async function saveKuorma() {
    ...
    // Säilytä mahdollinen jo saatu kuormanumero (palautettu aiemmasta onnistuneesta tallennuksesta)
...
const startKmForRow = (i < 6) ? String(kmA || "").trim() : String(kmB || "").trim();
    ...
    try {
      ...
      // Onnistuneella tallennuksella Apps Script palauttaa kuormanumeron.
      if (res.ok && ok && j && j.loadNo != null && String(j.loadNo).trim() !== "") {
        row.data = row.data || {};
        row.data.loadNo = String(j.loadNo).trim();
      }
