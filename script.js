...
const SHEETS_URL = "https://script.google.com/macros/s/AKfycbzGsq5vgnIdxN-fMKVYFcBHXWIa_lnGgwQNKbdC5IVJKSyabUk3e5ksLJAARl6M1RxJ/exec";
// ===============================
...
(function () {
  ...
  async function sendRow(index) {
    ...
    try {
      ...
      if (!(res.ok && ok) && parsed && ["invalid_token", "aud_mismatch", "forbidden_email", "unknown_browser", "invalid_browser", "missing_auth"].includes(parsed.error)) {
  ...
  async function fetchNextLoadNo(plate) {
    throw new Error("loadno_on_save_only");
  }
