...
(function () {
  ...
  function init() {
    ...
    // Kuorma nro (autokohtainen juokseva). Haetaan backendistä vain kerran per rivi.
    if (loadNoEl) {
      if (d.loadNo) {
        loadNoEl.value = String(d.loadNo);
      } else {
        loadNoEl.value = "";
        try {
          ...
          console.warn("loadNo fetch failed", err);
        }
      }
    }
...
const startKmForRow = (i < 6) ? String(kmA || "").trim() : String(kmB || "").trim();
  ...
  async function fetchNextLoadNo_(plate) {
    ...
    const url = `${SHEETS_URL}?action=nextLoadNumber&plate=${encodeURIComponent(p)}`;
    const res = await fetch(url, { method: "GET" });
    const txt = await res.text();
    let j = null;
    try { j = JSON.parse(txt); } catch {}
    if (!res.ok || !j || j.ok !== true || !j.loadNo) {
      const err = (j && j.error) ? j.error : (txt || "bad_response");
      throw new Error(err);
    }
    return String(j.loadNo);
  }
