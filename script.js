/
    addToList_(STORAGE_PRODUCTS, row.data?.tuote);
    fillDatalist_("tuoteDatalist", loadList_(STORAGE_PRODUCTS, []));
/ ===============================
// CONFIG
...
function updateKmUI() {
  // Yläpalkki näyttää viimeisimmän syötetyn km:n
  const v = String(state.lastKm || aloitusKm?.value || "").trim();
  if (aloitusKm) aloitusKm.value = v;
...
function loadList_(key, fallback=[]) {
  try { return JSON.parse(localStorage.getItem(key) || "") || fallback; } catch { return fallback; }
}
function saveList_(key, arr) {
  localStorage.setItem(key, JSON.stringify(arr || []));
}
function addToList_(key, value) {
  const v = String(value || "").trim();
  if (!v) return;
  const arr = loadList_(key, []);
  if (arr.includes(v)) return;
  arr.unshift(v);
  if (arr.length > 50) arr.length = 50;
  saveList_(key, arr);
}
function fillDatalist_(id, arr) {
...
function ensureStartKmAtLoad6(rowIndex0Based) {
  ...
  // Kysytään AINA kun avataan kuorma 6
  const cur = String(state.kmB || state.lastKm || "").trim();
  const input = prompt("Syötä aloitus km (kuormat 7–12):", cur);
  if (input === null) return true; // peruttiin -> pidä vanha
...
function onTableClick(e) {
    ...
    state.lastOpenedIndex = currentIndex + 1;
    persist();
...
function getAloitusKmForRow_(rowNo){
  const n = Number(rowNo)||1;
  if (n <= 6) {
    let v = String(state.kmA || "").trim();
    if (!v && n === 1) {
      const input = prompt("Syötä aloitus km (kuormat 1–6):", String(state.lastKm || "").trim());
      if (input === null) throw new Error("missing_kmA_cancel");
      v = String(input).trim();
      if (!v) throw new Error("missing_kmA_empty");
      state.kmA = v;
      state.lastKm = v;
      persist();
      updateKmUI();
  ...
  return String(state.kmB || "").trim();
}


async function ensureLoadNo_(row){
  if (row?.data?.loadNo) return row.data.loadNo;
  try{
    const url = (SHEETS_URL || "").trim();
    if (!url) return "";
    const res = await fetch(url + "?action=nextLoadNumber", { method:"GET" });
    const j = await res.json().catch(()=>null);
    const n = j && j.ok && j.next ? String(j.next) : "";
    row.data = row.data || {};
    row.data.loadNo = n;
    persist();
    return n;
...
async function sendRow(i) {
    ...
    row.status = "sending";
    await ensureLoadNo_(row);
    renderTable();

    const header = {
      pvm: (pvm?.value || "").trim(),
      auto: (auto?.value || "").trim(),
      aloitusKm: (function(){ try { return getAloitusKmForRow_(i+1); } catch(e){ alert("Lisää aloitus km ennen tallennusta."); throw e; } })(),
    };
