...
(function () {
  ...
  const loadNoLabel = $("loadNoLabel");

  let rows = [];
  let currentIndex = -1;

  // Aloitus KM kahdessa jaksossa
  let kmA = ""; // kuormat 1–6
  ...
  function init() {
    ...
    if (saved && Array.isArray(saved.rows) && saved.rows.length === ROWS_COUNT) {
      ...
      kmA = (saved.kmA != null ? String(saved.kmA) : String(saved.aloitusKm || "")).trim();
      kmB = (saved.kmB != null ? String(saved.kmB) : "").trim();
      lastKm = (saved.lastKm != null ? String(saved.lastKm) : String(saved.aloitusKm || "")).trim();
    ...
    btnSetKm?.addEventListener("click", () => {
      ...
      const inSecondHalf = (currentIndex >= 6) || !!kmB;
      if (inSecondHalf) kmB = vv; else kmA = vv;
      lastKm = vv;
      persist();
      updateKmUI();
    });
...
modal?.addEventListener("click", (e) => {
  ...
  // ---------- LOAD NUMBER (autokohtainen juokseva) ----------
  async function fetchNextLoadNo_(plate) {
    const p = String(plate || "").trim();
    if (!p) return null;
    if (!requireAuthOrAlert()) return null;

    const payload = {
      action: "nextLoadNumber",
      plate: p,
    ...
    try {
      ...
      if (j && j.ok === true && j.loadNo != null && String(j.loadNo).trim() !== "") {
        return String(j.loadNo).trim();
      }
    } catch (e) {
      console.warn("fetchNextLoadNo failed", e);
    }
    return null;
  }

  async function ensureRowLoadNo_(row) {
    if (!row) return;
    row.data = row.data || {};
    if (row.data.loadNo != null && String(row.data.loadNo).trim() !== "") return;

    const plate = (auto?.value || "").trim();
    if (!plate) return;

    const ln = await fetchNextLoadNo_(plate);
    if (!ln) return;

    row.data.loadNo = ln; // ei muokattavissa UI:ssa
    persist();
    // taulukkoon ei tarvitse piirtää numeroa, mutta status voi päivittyä
  }
  ...
  async function onTableClick(e) {
    ...
    await ensureRowLoadNo_(row); // hakee autokohtaisen juoksevan numeron tarvittaessa
    openModal(row);
  }
  ...
  function openModal(row) {
    if (!row) return;
    const d = row.data || {};
    if (loadNoLabel) loadNoLabel.textContent = (d.loadNo != null && String(d.loadNo).trim() !== "") ? String(d.loadNo).trim() : "—";
    const today = isoDate(new Date());
  ...
  async function saveKuorma() {
    ...
    saveLocation("tuoteList", d.tuote);

    // Säilytä mahdollinen jo haettu kuormanumero
    d.loadNo = (rows[currentIndex].data && rows[currentIndex].data.loadNo) ? rows[currentIndex].data.loadNo : d.loadNo;

    rows[currentIndex].data = d;

    await ensureRowLoadNo_(rows[currentIndex]);
...
// --- KM-logiikka ---
if (i === 0) {
  const curA = String(kmA || "").trim();
  if (!curA) {
    const v = prompt("Syötä aloituskilometrit (kuormat 1–6):", String(aloitusKm?.value || "").trim());
    if (!v) { row.status = "fail"; renderTable(); return; }
    kmA = String(v).trim();
    lastKm = kmA;
    if (aloitusKm) aloitusKm.value = lastKm;
    persist();
...
if (i === 5) {
  const v = prompt("Syötä aloituskilometrit (kuormat 7–12):", String(kmB || "").trim());
  if (!v) { row.status = "fail"; renderTable(); return; }
  kmB = String(v).trim();
  lastKm = kmB;
  if (aloitusKm) aloitusKm.value = lastKm;
...
if (i >= 6) {
  const curB = String(kmB || "").trim();
  if (!curB) {
    const v = prompt("Syötä aloituskilometrit (kuormat 7–12):", String(aloitusKm?.value || "").trim());
    if (!v) { row.status = "fail"; renderTable(); return; }
    kmB = String(v).trim();
    lastKm = kmB;
    if (aloitusKm) aloitusKm.value = lastKm;
...
const startKmForRow = (i < 6) ? String(kmA || "").trim() : String(kmB || "").trim();
